...
aipick 部署记录:
 ...
 - 版本号: 0.1.151.kim
 - 发布目录: /opt/sub2api/releases/20260711-081909
 - 当前二进制 SHA256: d52909a7ec32a24ddc0d85829c577ed23bb992dbdcf3e3fb71dcecc31c1e5ca0
 - 上一版发布目录: /opt/sub2api/releases/20260710-161211
 - 上一版二进制 SHA256: 8a6db6452f44bb4c19ebac17d9b7d3c571435980d061000e207f71cac210b006
 - 部署动作: 合并并保留 fork 定制提交后确认本地 HEAD 包含 `upstream/main`，构建前端，编译 linux/amd64 `-tags embed` 后端，压缩部署包并按 2 MiB 分块上传；首次上传在第 10 块中断后从断点续传，远端合并校验归档 SHA256 `814ffb7feed46dca6be6018bbbc63531f6b13391442e54d0aa3077fe32bb94e8` 和二进制 SHA256，同步 resources，切换 symlink，重启 `sub2api`
 - 验证结果: 本机 `http://127.0.0.1:8080/status` 返回 `{"status":"perfectly nice"}`；所有反向代理 HTTPS 入口 `/status` 与 `/` 均返回 200；运行版本为 `0.1.151.kim`；`systemctl show sub2api` 显示 ActiveState=active、SubState=running、NRestarts=0
 - 备注: 部署前远端运行版本为 `0.1.150.kim`，指向 `/opt/sub2api/releases/20260710-161211/sub2api`。重启时旧进程因活跃长连接未在 5 秒优雅停机期限内结束，记录一次 `context deadline exceeded` 和 exit status 1；systemd 随即正常启动新版，新进程持续处理请求且未发生重启。
 ...
 - 部署时间: 2026-07-08 07:41 Asia/Shanghai
 - Git HEAD: 9925f9dd30580aee2454ed73114a6b56cdf4de83
 - 版本号: 0.1.146.kim
 - 发布目录: /opt/sub2api/releases/20260708-073913
 - 当前二进制 SHA256: e8065212c3601344dadbd02ed3de36c7383e43e254521d1c024a115af1f23ed2
 - 上一版发布目录: /opt/sub2api/releases/20260707-011442
 - 上一版二进制 SHA256: e7fc8345faf1ecea9081dcaf1e92b43ff9480c0b3f259c0304edd365a58299bb
 - 部署动作: 构建前端，编译 linux/amd64 `-tags embed` 后端，压缩部署包并按 2 MiB 分块上传，远端合并校验归档 SHA256 `69d2c2bfb0794d1534682e72ff597312c4771cbfb1f4be501c07a63339810479` 和二进制 SHA256，同步 resources，切换 symlink，重启 `sub2api`
 - 验证结果: 本机 `http://127.0.0.1:8080/status` 返回 `{"status":"perfectly nice"}`；所有反向代理 HTTPS 入口 `/status` 与 `/` 均返回 200；`systemctl show sub2api` 显示 ActiveState=active、SubState=running、NRestarts=0
 - 备注: 部署前远端实际运行版本为 `0.1.145.kim`，指向 `/opt/sub2api/releases/20260707-011442/sub2api`。
